...
Файл **`.env.local`** використовується лише на комп’ютері під **`npm run dev`**. На **хостингу** статичний сайт викликає **`/api/order.php`** — PHP **не читає** `.env.local` з репозиторію.

Щоб заявки потрапляли в SalesDrive з продакшен-сайту, у **панелі хостингу** (змінні середовища для сайту / PHP) потрібно задати ті самі **`SALESDRIVE_API_KEY`** і **`SALESDRIVE_DOMAIN`**, щоб їх бачив `getenv()` або `$_SERVER`. Якщо цих змінних немає, скрипт зберігає заявку лише в **`storage/orders.jsonl`** і все одно відповідає «успіх» — у CRM нічого не з’явиться.
